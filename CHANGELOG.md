...
- game.FFd20.utils.measureReachDistance obsoleted.
- game.FFd20.utils.measureDistance introduced.
...
- Make game.FFd20.ItemAttack actually moddable ([1220](https://gitlab.com/Furyspark/foundryvtt-pathfinder1/-/issues/1220))
...
- Untangled items' useAttack and put the individual functions in game.FFd20.ItemAttack
...
- Prevent unnecessary `FFd20.getRollData` hook calls ([1134](https://gitlab.com/Furyspark/foundryvtt-pathfinder1/-/issues/1134))
...
- Added the hook "FFd20.gainXp" for adding experience through the ExperienceDistributor
- Removed unused .concentration from spellbooks.
- Add `FFd20.postInit` hook ([1045](https://gitlab.com/Furyspark/foundryvtt-pathfinder1/-/issues/1045))
...
- Added classes to all FFd20 dialogs: `add-character-class, apply-hit-points, use-attack, roll-initiative, duplicate-initiative, get-number, get-actor, die-roll, damage-roll, create-consumable`
...
- Added `flags.FFd20.subject` to chat message data to know what it's about (thanks mkahvi!)
...
- Added a hook for activating and deactivating of buffs which are active on actors keyed `FFd20.toggleActorBuff`. See the system's GitLab wiki for more information.
- Added a hook for activating and deactivating of conditions on actors keyed `FFd20.toggleActorCondition`. See the system's GitLab wiki for more information.
...
- Added the `FFd20.getRollData` hook. See the system's GitLab wiki for more information. (#769)
...
- An error prevented modules from extending FFd20 classes under some circumstances (e.g. Forge hosting)
- `ActorSheetPF#currentPrimaryTab` will now report tabs that aren't inventory as well
- Add `game.FFd20.utils` containing dialogGetActor, dialogGetNumber, createTag, getItemOwner, getActorFromId, getChangeFlat, convertDistance, convertWeight, convertWeightBack, and measureReachDistance
...
- Due to recent workflow changes, most FFd20 classes will now need to be imported from `/systems/FFd20/FFd20.js`, instead of `/systems/FFd20/modules/**/*.js`
  - Some modules referencing FFd20 classes will need an update before they work again (many apologies)
...
- Added error catcher to FFd20 socket calls (thanks mkahvi!)
...
- Compendiums from modules containing FFd20 specific entities were not migrated
...
- Dan Gomme greatly improved the FFd20 style measure templates (many thanks!)
...
- Add or update some items to the following compendiums:
  - Magic Items
  - Common Buffs
- Add macro'able function to show an actor's defenses as a chatlog (game.FFd20.rollDefenses()) (see [documentation](https://furyspark.gitlab.io/foundryvtt-pathfinder1-doc/advanced/macros/))
